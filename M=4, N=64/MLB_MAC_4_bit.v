module MLB_4(output signed [20:0] mlb,input [15:0] alpha_x,alpha_w,input [255:0] axi,awi);
    genvar i,j;
    wire signed [16:0] out[3:0][3:0];
    generate
        for(i=0; i<4; i=i+1) begin
            for(j=0; j<4; j=j+1) begin
                MLB_unit u_ij(.out(out[i][j]),.alpha_x(alpha_x[i*4+3:i*4]),.alpha_w(alpha_w[j*4+3:j*4]),.axi(axi[i*64+63:64*i]),.awi(awi[j*64+63:64*j]));
            end
        end
    endgenerate
    //reduction tree
