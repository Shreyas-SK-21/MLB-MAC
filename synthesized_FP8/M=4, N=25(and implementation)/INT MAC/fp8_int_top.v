module fp8_int_top (
    ...
    // Mantissas stay UNSIGNED [0..15]. Sign is handled AFTER multiplication inside int_mac_25,
    ...
    reg [199:0] fp8_activations_d1;
    reg [199:0] fp8_weights_d1;
    always @(posedge clk) begin
        fp8_activations_d1 <= fp8_activations;
        fp8_weights_d1     <= fp8_weights;
    end

    reg [3:0]  a_flat_reg  [0:24]; // 4-bit unsigned mantissa per lane
    reg [3:0]  b_flat_reg  [0:24]; // 4-bit unsigned mantissa per lane
    reg [24:0] sign_flat_reg;      // product sign per lane: sign_x XOR sign_w

    wire [99:0] a_flat_wire;
    wire [99:0] b_flat_wire;
    ...
    always @(*) begin
        for (i = 0; i < 25; i = i + 1) begin
            ...
            mant_x = {axi_planes[75+i], axi_planes[50+i], axi_planes[25+i], axi_planes[i]};
            mant_w = {awi_planes[75+i], awi_planes[50+i], awi_planes[25+i], awi_planes[i]};
    ...
    genvar k;
    generate
        for (k = 0; k < 25; k = k + 1) begin : pack_flat
