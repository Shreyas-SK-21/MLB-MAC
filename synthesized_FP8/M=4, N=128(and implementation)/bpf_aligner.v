module bfp_aligner (
    ...
    wire [3:0] exps_w [0:127];
    generate
        for (g=0; g<128; g=g+1) begin : extract_exp
    ...
    genvar m;
    generate
        for(m=0; m<64; m=m+1) assign maxL1[m] = (exps_w[2*m] > exps_w[2*m+1]) ? exps_w[2*m] : exps_w[2*m+1];
        for(m=0; m<32; m=m+1) assign maxL2[m] = (maxL1[2*m] > maxL1[2*m+1]) ? maxL1[2*m] : maxL1[2*m+1];
        for(m=0; m<16; m=m+1) assign maxL3[m] = (maxL2[2*m] > maxL2[2*m+1]) ? maxL2[2*m] : maxL2[2*m+1];
        for(m=0; m<8; m=m+1) assign maxL4[m] = (maxL3[2*m] > maxL3[2*m+1]) ? maxL3[2*m] : maxL3[2*m+1];
        for(m=0; m<4; m=m+1) assign maxL5[m] = (maxL4[2*m] > maxL4[2*m+1]) ? maxL4[2*m] : maxL4[2*m+1];
        for(m=0; m<2; m=m+1) assign maxL6[m] = (maxL5[2*m] > maxL5[2*m+1]) ? maxL5[2*m] : maxL5[2*m+1];
    endgenerate

    wire [3:0] max_exp_c = (((maxL6[0]) > (maxL6[1])) ? (maxL6[0]) : (maxL6[1]));

    // ---- Phase 2: parallel per-lane mantissa extraction, shift, corner-turn ----
    wire [3:0] mants_w        [0:127];
    wire [3:0] shifted_mant_w [0:127];
    generate
        for (g=0; g<128; g=g+1) begin : align_lane
    ...
    wire [511:0] aligned_planes_c;
    generate
        for (g=0; g<128; g=g+1) begin : corner_turn
            // CORNER TURN: map the 4-bit integer into the 4 planes for MLB_4
            assign aligned_planes_c[g]         = shifted_mant_w[g][0]; // Plane 0 (axi[127:0])
            assign aligned_planes_c[128 + g]  = shifted_mant_w[g][1]; // Plane 1 (axi[255:128])
            assign aligned_planes_c[256 + g] = shifted_mant_w[g][2]; // Plane 2 (axi[383:256])
            assign aligned_planes_c[384 + g] = shifted_mant_w[g][3]; // Plane 3 (axi[511:384])
        end
    endgenerate
