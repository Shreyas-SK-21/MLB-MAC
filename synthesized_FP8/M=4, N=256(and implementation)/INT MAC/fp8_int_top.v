module fp8_int_top (
    ...
    input  [2047:0] fp8_activations,
    input  [2047:0] fp8_weights,
    ...
    // Mantissas stay UNSIGNED [0..15]. Sign is handled AFTER multiplication inside int_mac_256,
    ...
    reg [2047:0] fp8_activations_d1;
    reg [2047:0] fp8_weights_d1;
    always @(posedge clk) begin
        fp8_activations_d1 <= fp8_activations;
        fp8_weights_d1     <= fp8_weights;
    end

    reg [3:0]  a_flat_reg  [0:255]; // 4-bit unsigned mantissa per lane
    reg [3:0]  b_flat_reg  [0:255]; // 4-bit unsigned mantissa per lane
    reg [255:0] sign_flat_reg;      // product sign per lane: sign_x XOR sign_w

    wire [1023:0] a_flat_wire;
    wire [1023:0] b_flat_wire;
    ...
    always @(*) begin
        for (i = 0; i < 256; i = i + 1) begin
            ...
            mant_x = {axi_planes[768+i], axi_planes[512+i], axi_planes[256+i], axi_planes[i]};
            mant_w = {awi_planes[768+i], awi_planes[512+i], awi_planes[256+i], awi_planes[i]};
    ...
    genvar k;
    generate
        for (k = 0; k < 256; k = k + 1) begin : pack_flat
