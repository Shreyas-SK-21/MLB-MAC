...
//
// Block 2 : Reduction tree 64 -> 1         (16-bit signed result)
//
...
//   a_flat   -- 64 x 4-bit UNSIGNED activations (256-bit bus)
//   b_flat   -- 64 x 4-bit UNSIGNED weights     (256-bit bus)
//   sign_flat-- 64 x 1-bit product sign         (sign_x XOR sign_w)
...
module int_mac_64 (
    ...
    // (BLOCK 0) to avoid a single flat net driving all 64 accumulator
    // lanes -- see gen_int_mac_n() patch, added to fix max_fanout/max_cap
    // violations on 'rst' (fanout ~64*22, e.g. 5633 @ N=256) and 'load'.

    input [255:0] a_flat,    // 64 x 4-bit UNSIGNED activations
    input [255:0] b_flat,    // 64 x 4-bit UNSIGNED weights
    input [63:0]  sign_flat, // 64 x 1-bit product sign (sign_x XOR sign_w)
...
// 'rst' or 'load' net driving all 64 accumulator lanes blows past
...
localparam GROUP = 16;
localparam NGRP  = (64 + GROUP - 1) / GROUP;
...
assign s_final = $signed(s5[0]) + $signed(s5[1]);
