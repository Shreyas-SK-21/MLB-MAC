...
module FP8_NATIVE_MAC_64 (
    ...
    // ====================================================================
    // ---- Reduction level 1: 64 -> 32 (width 37 -> 38) ----
    wire signed [37:0] sum_L1 [0:31];
    genvar j_L1;
    generate
        for (j_L1=0; j_L1<32; j_L1=j_L1+1) begin : lvl1_pair
            assign sum_L1[j_L1] = $signed(shifted_prod[2*j_L1]) + $signed(shifted_prod[2*j_L1+1]);
        end
    endgenerate
    // ---- Reduction level 2: 32 -> 16 (width 38 -> 39) ----
    wire signed [38:0] sum_L2 [0:15];
    genvar j_L2;
    generate
        for (j_L2=0; j_L2<16; j_L2=j_L2+1) begin : lvl2_pair
            assign sum_L2[j_L2] = $signed(sum_L1[2*j_L2]) + $signed(sum_L1[2*j_L2+1]);
        end
    endgenerate
    // ---- Reduction level 3: 16 -> 8 (width 39 -> 40) ----
    wire signed [39:0] sum_L3 [0:7];
    genvar j_L3;
    generate
        for (j_L3=0; j_L3<8; j_L3=j_L3+1) begin : lvl3_pair
            assign sum_L3[j_L3] = $signed(sum_L2[2*j_L3]) + $signed(sum_L2[2*j_L3+1]);
        end
    endgenerate
    // ---- Reduction level 4: 8 -> 4 (width 40 -> 41) ----
    wire signed [40:0] sum_L4 [0:3];
    genvar j_L4;
    generate
        for (j_L4=0; j_L4<4; j_L4=j_L4+1) begin : lvl4_pair
            assign sum_L4[j_L4] = $signed(sum_L3[2*j_L4]) + $signed(sum_L3[2*j_L4+1]);
        end
    endgenerate
    // ---- Reduction level 5: 4 -> 2 (width 41 -> 42) ----
    wire signed [41:0] sum_L5 [0:1];
    genvar j_L5;
    generate
        for (j_L5=0; j_L5<2; j_L5=j_L5+1) begin : lvl5_pair
            assign sum_L5[j_L5] = $signed(sum_L4[2*j_L5]) + $signed(sum_L4[2*j_L5+1]);
        end
    endgenerate
    // ---- Reduction level 6: 2 -> 1 (width 42 -> 43) ----
    wire signed [42:0] sum_L6 [0:0];
    genvar j_L6;
    generate
        for (j_L6=0; j_L6<1; j_L6=j_L6+1) begin : lvl6_pair
