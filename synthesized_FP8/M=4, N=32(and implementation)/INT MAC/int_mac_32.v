// ============================================================
// Full Integer MAC -- M=4, N=K=32  (Sign-After-Multiply version)
//
...
//
// Block 1 : 32 parallel unsigned MAC lanes
...
//
// Block 2 : Reduction tree 32 -> 1         (15-bit signed result)
//
...
//   a_flat   -- 32 x 4-bit UNSIGNED activations (128-bit bus)
//   b_flat   -- 32 x 4-bit UNSIGNED weights     (128-bit bus)
//   sign_flat-- 32 x 1-bit product sign         (sign_x XOR sign_w)
...
module int_mac_32 (
    ...
    // (BLOCK 0) to avoid a single flat net driving all 32 accumulator
    // lanes -- see gen_int_mac_n() patch, added to fix max_fanout/max_cap
    // violations on 'rst' (fanout ~32*22, e.g. 5633 @ N=256) and 'load'.

    input [127:0] a_flat,    // 32 x 4-bit UNSIGNED activations
    input [127:0] b_flat,    // 32 x 4-bit UNSIGNED weights
    input [31:0]  sign_flat, // 32 x 1-bit product sign (sign_x XOR sign_w)
...
// 'rst' or 'load' net driving all 32 accumulator lanes blows past
...
localparam GROUP = 16;
localparam NGRP  = (32 + GROUP - 1) / GROUP;
...
// ============================================================
// BLOCK 1 -- 32 parallel unsigned Multiply + sign + Accumulate
//
...
wire [7:0]         product       [0:31]; // unsigned magnitude product
wire signed [8:0]  signed_product[0:31]; // sign applied after multiply
reg  signed [10:0] acc           [0:31]; // accumulator, 11-bit for M=4

genvar i;
generate
    for (i = 0; i < 32; i = i + 1) begin : gen_mac_lane
...
// ============================================================
// BLOCK 2 -- Reduction tree 32 -> 1
// acc = 11-bit signed, each level adds 1 guard bit
// ============================================================

    wire signed [11:0] s1 [0:15];
    wire signed [12:0] s2 [0:7];
    wire signed [13:0] s3 [0:3];
    wire signed [14:0] s4 [0:1];

    genvar j;
    generate
        for (j = 0; j < 16; j = j + 1) begin : gen_r1
            assign s1[j] = $signed(acc[2*j]) + $signed(acc[2*j+1]);
        end
        for (j = 0; j < 8; j = j + 1) begin : gen_r2
            assign s2[j] = $signed(s1[2*j]) + $signed(s1[2*j+1]);
        end
        for (j = 0; j < 4; j = j + 1) begin : gen_r3
            assign s3[j] = $signed(s2[2*j]) + $signed(s2[2*j+1]);
        end
        for (j = 0; j < 2; j = j + 1) begin : gen_r4
...
// s_final : signed 15-bit
// scaled  : 24-bit (truncated to 21)
// ============================================================
