...
module tb_fp8_mlb_top;
    ...
    reg  [255:0] fp8_activations, fp8_weights;
    wire signed [17:0] wide_integer_sum;
    ...
    // ------------------------------------------------------------------
    reg [7:0] act_lane [0:31];
    reg [7:0] wt_lane  [0:31];
...
task fill_random_lanes(input allow_zero);
    ...
    // ------------------------------------------------------------------
    // Pack the 32 lanes into the 256-bit DUT inputs
    // ------------------------------------------------------------------
    task pack_vectors;
        integer i;
        begin
    ...
    // ------------------------------------------------------------------
task compute_reference(output reg signed [17:0] ref_result,
...
task apply_and_check(input [8*40-1:0] name);
        reg signed [17:0] expected_result;
    ...
    initial begin
        ...
        // 9: single nonzero lane among 31 zero lanes
        clear_all_lanes;
