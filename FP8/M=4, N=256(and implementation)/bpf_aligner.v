module bfp_aligner (
    ...
    wire [3:0] exps_w [0:255];
    generate
        for (g=0; g<256; g=g+1) begin : extract_exp
            assign exps_w[g] = fp8_vec[(g*8)+3 +: 4];
        end
    endgenerate

    // ---- Phase 1b: log-depth max-reduction tree ----
    wire [3:0] maxL1 [0:127];
    wire [3:0] maxL2 [0:63];
    wire [3:0] maxL3 [0:31];
    wire [3:0] maxL4 [0:15];
    wire [3:0] maxL5 [0:7];
    ...
    genvar m;
    generate
        for(m=0; m<128; m=m+1) assign maxL1[m] = (exps_w[2*m] > exps_w[2*m+1]) ? exps_w[2*m] : exps_w[2*m+1];
        for(m=0; m<64; m=m+1) assign maxL2[m] = (maxL1[2*m] > maxL1[2*m+1]) ? maxL1[2*m] : maxL1[2*m+1];
        for(m=0; m<32; m=m+1) assign maxL3[m] = (maxL2[2*m] > maxL2[2*m+1]) ? maxL2[2*m] : maxL2[2*m+1];
        for(m=0; m<16; m=m+1) assign maxL4[m] = (maxL3[2*m] > maxL3[2*m+1]) ? maxL3[2*m] : maxL3[2*m+1];
        for(m=0; m<8; m=m+1) assign maxL5[m] = (maxL4[2*m] > maxL4[2*m+1]) ? maxL4[2*m] : maxL4[2*m+1];
        for(m=0; m<4; m=m+1) assign maxL6[m] = (maxL5[2*m] > maxL5[2*m+1]) ? maxL5[2*m] : maxL5[2*m+1];
        for(m=0; m<2; m=m+1) assign maxL7[m] = (maxL6[2*m] > maxL6[2*m+1]) ? maxL6[2*m] : maxL6[2*m+1];
    endgenerate

    wire [3:0] max_exp_c = (((maxL7[0]) > (maxL7[1])) ? (maxL7[0]) : (maxL7[1]));

    // ---- Phase 2: parallel per-lane mantissa extraction, shift, corner-turn ----
    wire [3:0] mants_w        [0:255];
    wire [3:0] shifted_mant_w [0:255];
    generate
        for (g=0; g<256; g=g+1) begin : align_lane
    ...
    wire [1023:0] aligned_planes_c;
    generate
        for (g=0; g<256; g=g+1) begin : corner_turn
            // CORNER TURN: map the 4-bit integer into the 4 planes for MLB_4
            assign aligned_planes_c[g]         = shifted_mant_w[g][0]; // Plane 0 (axi[255:0])
            assign aligned_planes_c[256 + g]  = shifted_mant_w[g][1]; // Plane 1 (axi[511:256])
            assign aligned_planes_c[512 + g] = shifted_mant_w[g][2]; // Plane 2 (axi[767:512])
            assign aligned_planes_c[768 + g] = shifted_mant_w[g][3]; // Plane 3 (axi[1023:768])
        end
    endgenerate
