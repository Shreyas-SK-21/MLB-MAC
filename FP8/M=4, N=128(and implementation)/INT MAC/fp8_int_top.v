module fp8_int_top (
    ...
    input  [1023:0] fp8_activations,
    input  [1023:0] fp8_weights,
    ...
    // Mantissas stay UNSIGNED [0..15]. Sign is handled AFTER multiplication inside int_mac_128,
    ...
    reg [1023:0] fp8_activations_d1;
    reg [1023:0] fp8_weights_d1;
    always @(posedge clk) begin
        fp8_activations_d1 <= fp8_activations;
        fp8_weights_d1     <= fp8_weights;
    end

    reg [3:0]  a_flat_reg  [0:127]; // 4-bit unsigned mantissa per lane
    reg [3:0]  b_flat_reg  [0:127]; // 4-bit unsigned mantissa per lane
    reg [127:0] sign_flat_reg;      // product sign per lane: sign_x XOR sign_w

    wire [511:0] a_flat_wire;
    wire [511:0] b_flat_wire;
    ...
    always @(*) begin
        for (i = 0; i < 128; i = i + 1) begin
            ...
            mant_x = {axi_planes[384+i], axi_planes[256+i], axi_planes[128+i], axi_planes[i]};
            mant_w = {awi_planes[384+i], awi_planes[256+i], awi_planes[128+i], awi_planes[i]};
    ...
    genvar k;
    generate
        for (k = 0; k < 128; k = k + 1) begin : pack_flat
