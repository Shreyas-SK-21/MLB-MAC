// ============================================================
// Full Integer MAC -- M=4, N=K=25  (Sign-After-Multiply version)
//
...
//
// Block 1 : 25 parallel unsigned MAC lanes
...
//
// Block 2 : Reduction tree 25 -> 1         (16-bit signed result)
//
...
//   a_flat   -- 25 x 4-bit UNSIGNED activations (100-bit bus)
//   b_flat   -- 25 x 4-bit UNSIGNED weights     (100-bit bus)
//   sign_flat-- 25 x 1-bit product sign         (sign_x XOR sign_w)
...
module int_mac_25 (
    ...
    input [24:0]  sign_flat, // 25 x 1-bit product sign (sign_x XOR sign_w)
...
wire [7:0]         product       [0:24]; // unsigned magnitude product
wire signed [8:0]  signed_product[0:24]; // sign applied after multiply
reg  signed [10:0] acc           [0:24]; // accumulator, 11-bit for M=4

genvar i;
generate
    for (i = 0; i < 25; i = i + 1) begin : gen_mac_lane
...
// ============================================================
// BLOCK 2 -- Reduction tree 25 -> 1
// acc = 11-bit signed, each level adds 1 guard bit
// ============================================================

    wire signed [11:0] s1 [0:11];
    wire signed [12:0] s2 [0:5];
    wire signed [13:0] s3 [0:2];
    wire signed [14:0] s4 [0:0];

    genvar j;
    generate
        for (j = 0; j < 12; j = j + 1) begin : gen_r1
            assign s1[j] = $signed(acc[2*j]) + $signed(acc[2*j+1]);
        end
        for (j = 0; j < 6; j = j + 1) begin : gen_r2
            assign s2[j] = $signed(s1[2*j]) + $signed(s1[2*j+1]);
        end
        for (j = 0; j < 3; j = j + 1) begin : gen_r3
            assign s3[j] = $signed(s2[2*j]) + $signed(s2[2*j+1]);
        end
        for (j = 0; j < 1; j = j + 1) begin : gen_r4
            assign s4[j] = $signed(s3[2*j]) + $signed(s3[2*j+1]);
        end
    endgenerate

wire signed [15:0] s_final;
assign s_final = $signed(s4[0]) + $signed({{{5{{acc[24][10]}}}}, acc[24]}) + $signed({{{2{{s3[2][13]}}}}, s3[2]});

// ============================================================
// BLOCK 3 -- Scaling ax*aw x sum + offset bxw
// ax, aw  : unsigned 4-bit -> product 8-bit unsigned
// s_final : signed 16-bit
// scaled  : 25-bit (truncated to 21)
// ============================================================
