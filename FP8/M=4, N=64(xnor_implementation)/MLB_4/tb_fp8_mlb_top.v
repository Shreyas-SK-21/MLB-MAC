...
module tb_fp8_mlb_top;
    ...
    // ------------------------------------------------------------------
    reg [7:0] act_lane [0:63];
    reg [7:0] wt_lane  [0:63];
    ...
    task set_lane(input integer is_weight, input integer idx,
                  input sign, input [3:0] exp_f, input [2:0] mant_f);
        begin
            if (is_weight)
    ...
    // ------------------------------------------------------------------
    // Pack the 64 lanes into the 512-bit DUT inputs
    // ------------------------------------------------------------------
    task pack_vectors;
        integer i;
        begin
    ...
    task apply_and_check(input [8*40-1:0] name);
        ...
        begin
            ...
            // Safe 1-cycle pulse using negedge
            @(negedge clk);
            valid_in = 1'b1;
            @(negedge clk);
            valid_in = 1'b0;
    ...
    initial begin
        ...
        // 2: all lanes = +1.0 x +1.0  (sign=0, exp=7, mant_frac=0)
        for (i = 0; i < 64; i = i + 1) begin
        ...
        // 3: all activations positive, all weights negative -> result negative
        for (i = 0; i < 64; i = i + 1) begin
            set_lane(0, i, 1'b0, 4'd7, 3'b000);
        ...
        // 4: both operands negative -> sign cancels to positive
        for (i = 0; i < 64; i = i + 1) begin
        ...
        // 5: alternating sign pattern on activations
        for (i = 0; i < 64; i = i + 1) begin
        ...
        // 7: max magnitude, alternating sign pairs
        for (i = 0; i < 64; i = i + 1) begin
        ...
        // 8: subnormal flush -- odd lanes exp=0 (must contribute 0)
        for (i = 0; i < 64; i = i + 1) begin
            if (i[0]) begin
                set_lane(0, i, 1'b0, 4'd0, 3'b101); // subnormal -> 0
                set_lane(1, i, 1'b0, 4'd0, 3'b011); // subnormal -> 0
            end else begin
        ...
        // 9: single nonzero lane among 63 zero lanes
        clear_all_lanes;
        set_lane(0, 0, 1'b0, 4'd10, 3'b110);
        set_lane(1, 0, 1'b1, 4'd10, 3'b110);
        apply_and_check("Single nonzero lane");

        // 10: wide dynamic range -- huge max lane, rest shifted to zero
        clear_all_lanes;
        ...
        // 11: all activations subnormal, weights normal -> result must be 0
        for (i = 0; i < 64; i = i + 1) begin
        ...
        // 12: single lane, exact product check
        clear_all_lanes;
        set_lane(0, 5, 1'b0, 4'd8, 3'b110);  // +1.110 * 2^1 aligned mantissa=14
        set_lane(1, 5, 1'b0, 4'd8, 3'b011);  // +1.011 * 2^1 aligned mantissa=11
        apply_and_check("Single lane exact product");

        // 13: mixed-exponent, varied signs
        clear_all_lanes;
        for (i = 0; i < 32; i = i + 1) begin
            set_lane(0, i, 1'b0, 4'd12, 3'b111); // large exponent acts
            set_lane(1, i, 1'b1, 4'd12, 3'b111); // large neg weights
        end
        for (i = 32; i < 64; i = i + 1) begin
            set_lane(0, i, 1'b1, 4'd10, 3'b101); // smaller neg acts
            set_lane(1, i, 1'b0, 4'd10, 3'b101); // smaller pos weights
        end
        apply_and_check("Mixed exponent and sign halves");

        // 14: identical vectors (all +2.0), pure positive
        for (i = 0; i < 64; i = i + 1) begin
            set_lane(0, i, 1'b0, 4'd8, 3'b000);
            set_lane(1, i, 1'b0, 4'd8, 3'b000);
        end
        apply_and_check("All +2.0 x +2.0");

        // 15: alternating large/small exponent on activations only
        for (i = 0; i < 64; i = i + 1) begin
            set_lane(0, i, 1'b0, i[0] ? 4'd15 : 4'd5, 3'b111);
            set_lane(1, i, 1'b0, 4'd10, 3'b111);
        end
        apply_and_check("Alternating large/small act exponent");

        // ============================================================
        // Randomized cases (50 tests, mixed normal/subnormal)
        // ============================================================
        for (t = 0; t < 50; t = t + 1) begin
            if (t % 3 == 0)
                fill_random_lanes(1'b1);   // allow zero-exponent (subnormals)
            else
