...
module tb_fp8_int_top;
    ...
    // ------------------------------------------------------------------
    // Behavioral reference model — IDENTICAL to fp8_mlb_top reference
    //   Full 4-bit mantissa, no right-shift, exp_sum = max_ex+max_ew-14
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // Drive DUT, wait for mac_done, compare, log result
    // ------------------------------------------------------------------
    task apply_and_check(input [8*40-1:0] name);
        ...
        begin
            ...
            end else if ((wide_integer_sum !== expected_result) ||
    ...
    // ------------------------------------------------------------------
    // Main stimulus  (same directed + random cases as MLB testbench)
    // ------------------------------------------------------------------
    integer t;
    integer i;
    initial begin
        ...
        clear_all_lanes;
        set_lane(0, 5, 1'b0, 4'd8, 3'b110);
        set_lane(1, 5, 1'b0, 4'd8, 3'b011);
