...
module FP8_NATIVE_MAC_25 (
    input clk,
    input rst,
    input valid_in,
    input [199:0] fp8_act,      // 25 parallel FP8 E4M3 activations
    input [199:0] fp8_wgt,      // 25 parallel FP8 E4M3 weights
    output reg done,
    output reg signed [41:0] final_acc // 42-bit exact sum (implicit scale 2^-18)
);

    // ====================================================================
    // STAGE 1: 25 Native FP8 Multipliers -> 37-bit Fixed-Point
    // ====================================================================
    reg signed [36:0] shifted_prod [0:24];
    reg stage1_valid;

    genvar i;
    generate
        for (i = 0; i < 25; i = i + 1) begin : fp8_mult_lane
    ...
    // ====================================================================
    // STAGE 2: 25-to-1 Integer Reduction Tree
    // ====================================================================
    // ---- Reduction level 1: 25 -> 13 (width 37 -> 38) ----
    wire signed [37:0] sum_L1 [0:12];
    genvar j_L1;
    generate
        for (j_L1=0; j_L1<12; j_L1=j_L1+1) begin : lvl1_pair
            assign sum_L1[j_L1] = $signed(shifted_prod[2*j_L1]) + $signed(shifted_prod[2*j_L1+1]);
        end
    endgenerate
    assign sum_L1[12] = $signed(shifted_prod[24]);
    // ---- Reduction level 2: 13 -> 7 (width 38 -> 39) ----
    wire signed [38:0] sum_L2 [0:6];
    genvar j_L2;
    generate
        for (j_L2=0; j_L2<6; j_L2=j_L2+1) begin : lvl2_pair
            assign sum_L2[j_L2] = $signed(sum_L1[2*j_L2]) + $signed(sum_L1[2*j_L2+1]);
        end
    endgenerate
    assign sum_L2[6] = $signed(sum_L1[12]);
    // ---- Reduction level 3: 7 -> 4 (width 39 -> 40) ----
    wire signed [39:0] sum_L3 [0:3];
    genvar j_L3;
    generate
        for (j_L3=0; j_L3<3; j_L3=j_L3+1) begin : lvl3_pair
