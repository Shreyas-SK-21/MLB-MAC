...
module tb_fp8_mlb_top;
    ...
    reg  [71:0] fp8_activations, fp8_weights;
...
task fill_random_lanes(input allow_zero);
    ...
    // ------------------------------------------------------------------
    // Pack the 9 lanes into the 72-bit DUT inputs
    // ------------------------------------------------------------------
    task pack_vectors;
        integer i;
        begin
...
task apply_and_check(input [8*40-1:0] name);
    ...
    initial begin
        ...
        // 9: single nonzero lane among 8 zero lanes
        clear_all_lanes;
