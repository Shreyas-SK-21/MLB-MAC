module fp8_int_top (
    input  clk,
    input  rst,
    input  valid_in, // Connects to 'load' on the MAC array
    input  [71:0] fp8_activations,
    input  [71:0] fp8_weights,
    output signed [20:0] wide_integer_sum,
    output signed [8:0]   shared_exponent
);

    // ---------------- BFP alignment (Reused from MLB design) ----------------
    wire [35:0] axi_planes;
    wire [35:0] awi_planes;
    wire [3:0]   max_exp_x;
    wire [3:0]   max_exp_w;

    bfp_aligner align_x (
        .clk(clk),
        .fp8_vec(fp8_activations),
        .aligned_planes(axi_planes),
        .max_exp(max_exp_x)
    );

    bfp_aligner align_w (
        .clk(clk),
        .fp8_vec(fp8_weights),
        .aligned_planes(awi_planes),
        .max_exp(max_exp_w)
    );

    // ---------------- Format for INT MAC (Plane-to-Flat, Sign Separate) ----------------
    // Mantissas stay UNSIGNED [0..15]. Sign is handled AFTER multiplication inside int_mac_9,
    // exactly like the FP8 MLB design -- avoids the >>1 truncation of the old approach.
    //
    // axi_planes/awi_planes now arrive ONE cycle late from the pipelined
    // aligner above. The sign-XOR term below is derived from the raw
    // (un-registered) fp8_activations/fp8_weights, so we register those
    // inputs by 1 cycle too -- otherwise sign_flat_reg would be computed
    // one cycle EARLIER than the mantissas it's supposed to pair with.
    reg [71:0] fp8_activations_d1;
    reg [71:0] fp8_weights_d1;
    always @(posedge clk) begin
        fp8_activations_d1 <= fp8_activations;
        fp8_weights_d1     <= fp8_weights;
    end

    reg [3:0]  a_flat_reg  [0:8]; // 4-bit unsigned mantissa per lane
    reg [3:0]  b_flat_reg  [0:8]; // 4-bit unsigned mantissa per lane
    reg [8:0] sign_flat_reg;      // product sign per lane: sign_x XOR sign_w

    wire [35:0] a_flat_wire;
    wire [35:0] b_flat_wire;

    integer i;
    reg [3:0] mant_x, mant_w;

    always @(*) begin
        for (i = 0; i < 9; i = i + 1) begin
            // 1. Reconstruct full 4-bit unsigned mantissa from BFP bit-planes
            mant_x = {axi_planes[27+i], axi_planes[18+i], axi_planes[9+i], axi_planes[i]};
            mant_w = {awi_planes[27+i], awi_planes[18+i], awi_planes[9+i], awi_planes[i]};

            a_flat_reg[i] = mant_x;
            b_flat_reg[i] = mant_w;

            // 2. Compute product sign (sign-magnitude rule: sign = XOR of input signs)
            //    using the delayed (aligned-in-time) activation/weight copies.
            sign_flat_reg[i] = fp8_activations_d1[i*8 + 7] ^ fp8_weights_d1[i*8 + 7];
        end
    end

    // Pack mantissa registers into flat buses
    genvar k;
    generate
        for (k = 0; k < 9; k = k + 1) begin : pack_flat
            assign a_flat_wire[4*k +: 4] = a_flat_reg[k];
            assign b_flat_wire[4*k +: 4] = b_flat_reg[k];
        end
    endgenerate

    // valid_in delayed by 1 cycle to match the aligner's registered
    // latency -- the INT MAC array must not start accumulating before
    // a_flat_wire/b_flat_wire/sign_flat_reg are actually valid.
    reg valid_in_d1;
    always @(posedge clk) begin
        if (rst) valid_in_d1 <= 1'b0;
        else     valid_in_d1 <= valid_in;
    end

    // ---------------- INT MAC Instantiation ----------------
    int_mac_9 mac_array (
        .clk(clk),
        .rst(rst),
        .load(valid_in_d1),
        .a_flat(a_flat_wire),
        .b_flat(b_flat_wire),
        .sign_flat(sign_flat_reg),   // per-lane product sign
        .alpha_x(4'd1),
        .alpha_w(4'd1),
        .beta_xw(8'sd0),
        .result(wide_integer_sum)
    );

    // ---------------- Final Exponent ----------------
    // No >>1 compensation needed anymore. Full 4-bit mantissas are used (hidden bit at
    // position 3, so each mantissa integer is 2^3 = 8x the true fraction).
    // Two mantissas multiplied -> 2^6 total scale -> same formula as FP8 MLB: bias*2 + 6 = 14+6 = 20
    assign shared_exponent = $signed({5'b0, max_exp_x}) + $signed({5'b0, max_exp_w}) - 9'sd20;

endmodule
