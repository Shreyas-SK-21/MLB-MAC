module MLB_4(output reg signed [27:0] mlb,output reg done,input [31:0] alpha_x,alpha_w,input [255:0] axi,awi,input clk,rst,valid_in);//xnor_popcount of axi and awi and alpha_x and alpha_w is the matrix constants
    ...
    genvar j;
    wire signed [23:0] unit_out[0:3];
    wire [3:0] sub_unit_done;
    generate
        for(j=0;j<4;j=j+1) begin
            MLB_unit u_j(.out(unit_out[j]),.done(sub_unit_done[j]),.alpha_x(multiplexed_alpha_x),.alpha_w(alpha_w[j*8+7:j*8]),.axi(multiplexed_axi),.awi(awi[j*64+63:64*j]),.clk(clk),.rst(rst),.valid_in(valid_in));
        end
    endgenerate

    // Maintaining original structure using reg arrays to accumulate across cycles
    reg signed [23:0] out[0:15];
    reg [15:0]unit_done;
    integer login;
    ...
    always @(posedge clk) begin
        if(rst) begin
            state <= 2'd0;
            unit_done <= 16'b0;
            for(login=0; login<16; login=login+1) out[login] <= 24'sd0;
        end
    ...
    // Reduction tree
    wire signed [24:0]s00,s01,s02,s03,s04,s05,s06,s07;
    wire signed [25:0]s10,s11,s12,s13;
    wire signed [26:0]s20,s21;
    wire signed [27:0]s30;
