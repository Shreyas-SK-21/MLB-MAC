...
from quantization import MLBConv2d, MLBLinear
...
def _make_conv(
    ...
    """Return either a plain Conv2d or an MLBConv2d based on `use_mlb`."""
    if use_mlb:
        return MLBConv2d(
            in_ch, out_ch,
...
def _make_linear(
    ...
    """Return either a plain Linear or an MLBLinear based on `use_mlb`."""
    if use_mlb:
...
class ResNet20(nn.Module):
    """
    ResNet-20 for Fashion-MNIST (1-channel, 28×28, 10 classes).
    ...
    mixed_precision_config : dict | None
        ...
        Example: {"conv1": 5, "layer1": 5, "layer2": 4, "layer3": 3, "fc": 2}
    num_classes : int
    ...
    def __init__(
        ...
        self.conv1 = _make_conv(
            in_ch=1, out_ch=16,
            kernel_size=3, stride=1, padding=1, bias=False,
            use_mlb=use_mlb, M=_M("conv1"), qat=qat,
        )
        ...
        self.layer1 = self._make_layer(
            ...
            blocks=3, stride=1,
            use_mlb=use_mlb, M=_M("layer1"), qat=qat,
        )
        # layer2 — 3 blocks, 32 channels, stride 2 (14×14)
        self.layer2 = self._make_layer(
            in_channels=16, out_channels=32,
            blocks=3, stride=2,
            use_mlb=use_mlb, M=_M("layer2"), qat=qat,
        )
        # layer3 — 3 blocks, 64 channels, stride 2 (7×7)
        self.layer3 = self._make_layer(
            in_channels=32, out_channels=64,
            blocks=3, stride=2,
            use_mlb=use_mlb, M=_M("layer3"), qat=qat,
        )
        ...
        self.fc = _make_linear(
            ...
            use_mlb=use_mlb, M=_M("fc"), qat=qat,
        )
    ...
    def _make_layer(
        ...
        # Remaining blocks keep spatial dims
...
def build_uniform_mlb_resnet20(
    ...
    Parameters
    ----------
    M   : uniform binary levels applied to all MLB layers
...
def build_mixed_precision_resnet20(
    ...
    Parameters
    ----------
    mixed_precision_config : dict  {group_name: M}
    qat                    : enable quantization-aware training
    """
    return ResNet20(
        use_mlb=True,
