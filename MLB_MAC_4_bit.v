...
module MLB #(
    ...
    output [UNIT_OUT_WIDTH+1:0] mlb,

    input [NUM_UNITS*UNIT_WIDTH-1:0] alpha_x,
    input [NUM_UNITS*UNIT_WIDTH-1:0] alpha_w,
    input [NUM_UNITS*UNIT_WIDTH-1:0] axi,
    input [NUM_UNITS*UNIT_WIDTH-1:0] awi
);

    localparam UNIT_OUT_WIDTH =
        (2*UNIT_WIDTH) + $clog2(UNIT_WIDTH+1);

    wire [NUM_UNITS*UNIT_OUT_WIDTH-1:0] out;

    genvar i;
    generate
        for(i = 0; i < NUM_UNITS; i = i + 1) begin : mlb_units
            ...
            MLB_unit #(
                ...
                .out(
                    out[i*UNIT_OUT_WIDTH +: UNIT_OUT_WIDTH]
                ),

                .alpha_x(
                    alpha_x[i*UNIT_WIDTH +: UNIT_WIDTH]
                ),

                .alpha_w(
                    alpha_w[i*UNIT_WIDTH +: UNIT_WIDTH]
                ),

                .axi(
                    axi[i*UNIT_WIDTH +: UNIT_WIDTH]
                ),

                .awi(
                    awi[i*UNIT_WIDTH +: UNIT_WIDTH]
                )
            );
    ...
    reduction_tree_4 #(
        ...
        .a0(out[0*UNIT_OUT_WIDTH +: UNIT_OUT_WIDTH]),
        .a1(out[1*UNIT_OUT_WIDTH +: UNIT_OUT_WIDTH]),
        .a2(out[2*UNIT_OUT_WIDTH +: UNIT_OUT_WIDTH]),
        .a3(out[3*UNIT_OUT_WIDTH +: UNIT_OUT_WIDTH])
    );
